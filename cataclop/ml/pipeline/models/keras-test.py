...
class Model(factories.Model):
    ...
    def load(self, force=False):
        if not self.loaded or force:
            #self.horse_dummies = load(open('/Users/pourquoi/workspace/cataclop/cataclop-django/var/models/keras-pair-pos/horse_dummies.pkl', 'rb'))
            #self.race_dummies = load(open('/Users/pourquoi/workspace/cataclop/cataclop-django/var/models/keras-pair-pos/race_dummies.pkl', 'rb'))
            ...
            #self.scaler = load(open('/Users/pourquoi/workspace/cataclop/cataclop-django/var/models/keras-pair-pos/scaler.pkl', 'rb'))
            #self.model = keras.models.load_model(os.path.join(self.data_dir, 'model.h5'))
            self.model = keras.models.load_model('/tmp/model')
            #self.model = keras.models.load_model('/Users/pourquoi/workspace/cataclop/cataclop-django/var/models/keras-pair-pos/model.h5')
            #self.model.set_weights(load(open('/tmp/weights.pkl', 'rb')))
    ...
    def prepare_data(self, dataset, train=True):
        ...
        if train:
            ...
            df = df.groupby('race_id').filter(lambda r: (r['trueskill_mu'] == 25).sum() < r['declared_player_count'].max()/2 and r['odds_0'].min() != dataset.params['nan_flag'] and r['odds_1'].min() != dataset.params['nan_flag'] )
        ...
        if train:
            ...
            scaled = self.scaler.transform(df.loc[:, features].values)
        ...
        self.all_horse_features = list(set(horse_features + list(df_horse_dummies.columns)))

        self.all_race_features = list(set(race_features + list(df_race_dummies.columns)))
    ...
    def make_Xy(self, df, train=True):
        ...
        for race_id, race in tqdm(races, total=n_races):
            ...
            for i in range(n_players):
                ...
                if train and (player1['position'] == 0 or player1['position'] >= 10 ):
                    continue
                ...
                for j in range(n_players):
                    ...
                    # only train on 4 first
                    if train and (player2['position'] == 0 or player2['position'] >= 10):
                        continue
